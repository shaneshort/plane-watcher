...
func TestSlipstreamVerifyPassesAfterWrite(t *testing.T) {
	repo := t.TempDir()
	recipeDir := filepath.Join(repo, "firmware", "yocto", "meta-plane-watcher", "recipes-pwtools", "plane-watcher-tools")
	if err := os.MkdirAll(recipeDir, 0o755); err != nil {
		t.Fatal(err)
	}

	inc := filepath.Join(recipeDir, "plane-watcher-tools-binaries.inc")
	cmds := []config.CommandMeta{
		{Name: "plane-feeder", Targets: []string{"arm"}},
	}
