...
The live RF environment is much messier than the block diagrams suggest. In the capture below, most of the time series is just low-level RF clutter; the narrow spikes are the occasional transponder bursts that rise far enough above the floor to decode. The faint gold markers show places where `dump1090` found valid Mode S frames in the same capture.

![1090 MHz RF Snapshot from g24_sc16.raw](./images/capture-rf-overview.svg)

This is the backdrop for the holdoff changes described on the next page. The detector does not operate on isolated textbook waveforms; it operates in a stream where payload energy, nearby aircraft, and front-end ringing can all create preamble-like structure a few microseconds after a real detection. Seeing the whole capture first makes that tradeoff easier to understand.
